	...
	DWORD inputLen) noexcept
{
	...
	auto index = ((_ctx.count[0] >> 3) & 0x3F);
	/* Update number of bits */
	if ((_ctx.count[0] += (inputLen << 3)) < (inputLen << 3))
		_ctx.count[1]++;

	_ctx.count[1] += (inputLen >> 29);
	const auto partLen = 64 - index;
	/* Transform as many times as possible.
	*/
	DWORD i = 0;
	if (inputLen >= partLen) {
		memcpy(&_ctx.buffer[index], &input[0], partLen);
		transform(&_ctx.state[0], &_ctx.buffer[0]);
		for (i = partLen; i + 63 < inputLen; i += 64)
			transform(&_ctx.state[0], &input[i]);
		index = 0;
	}
	else
		i = 0;
...
const BYTE* const CMD5::final() noexcept
{
	...
	/* Save number of bits */
	encode(&bits[0], &_ctx.count[0], 8);
	/* Pad out to 56 mod 64.
	*/
	const auto index = ((_ctx.count[0] >> 3) & 0x3f);
	const auto padLen = (index < 56) ? (56 - index) : (120 - index);
	update(&PADDING[0], padLen);
	/* Append length (before padding) */
	update(&bits[0], 8);
	/* Store state in digest */
	encode(&_hash[0], &_ctx.state[0], 16);
	...
	const CMD5& md5) noexcept
{
	...
	memcpy(&_hash[0], &md5._hash[0], sizeof(_hash));
